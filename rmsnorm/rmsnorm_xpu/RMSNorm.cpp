...
using namespace at::AtenTypeXPU::normalization;
...
void RmsNormBackwardKernelImplInternal(
  ...
  if (grad_input_mask[1]) {
    // backward weight
    Tensor x_norm = at::mul(X.to(at::kFloat), rstd.unsqueeze(1).to(at::kFloat));
    Tensor sum_tmp = at::mul(x_norm, dY.to(at::kFloat));
    at::sum_out(dgamma, sum_tmp, at::IntArrayRef{0});
    dgamma = dgamma.to(X.scalar_type());
  }
}
