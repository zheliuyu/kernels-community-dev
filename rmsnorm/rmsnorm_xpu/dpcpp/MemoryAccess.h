...
struct multi_outputs_store_helper {
  ...
  static void apply(
      kernels::xpu::dpcpp::Array<char*, ntensors> data,
      kernels::xpu::dpcpp::Array<uint32_t, num_outputs> offsets,
      std::tuple<Args...> ret) {
...
struct LoadWithCast {
  using array_t =
      kernels::xpu::dpcpp::Array<at::ScalarType, std::max<int>(N, 1)>;
  using size_array_t =
      kernels::xpu::dpcpp::Array<uint32_t, std::max<int>(N, 1)>;
...
static inline int preferred_vector_width(at::DeviceIndex dev_id, int elem_sz) {
  ...
  switch (elem_sz) {
    case 1:
      ...
      ret = kernels::xpu::dpcpp::dpcppPrefVectorWidth<char>(dev_id);
      break;
    case 2:
      static_assert(sizeof(short) == 2, "the short size is not 2 bytes");
      ret = kernels::xpu::dpcpp::dpcppPrefVectorWidth<short>(dev_id);
      break;
    case 4:
      ret = kernels::xpu::dpcpp::dpcppPrefVectorWidth<int>(dev_id);
      static_assert(sizeof(int) == 4, "the long size is not 4 bytes");
      break;
    case 8:
      static_assert(sizeof(int64_t) == 8, "the long size is not 8");
      ret = kernels::xpu::dpcpp::dpcppPrefVectorWidth<int64_t>(dev_id);
      break;
    default:
      // no vectorize
      ret = 1;
