...
static inline c10::ScalarType opaqueTypeToScalarType(const at::Tensor& tensor) {
  ...
  auto ctx = *(static_cast<at::AtenTypeXPU::DPCPPTensorContext*>(
...
inline bool onednn_strides_check(const Tensor& src) {
  auto adims = kernels::xpu::oneDNN::get_onednn_dims(src);
  int ndims = (int)adims.size();
  auto dims = adims.data();

  auto data_type = static_cast<dnnl_data_type_t>(
      kernels::xpu::oneDNN::get_onednn_dtype_include_double(
          src, /*allow_undef*/ false));
  auto strides_info = kernels::xpu::oneDNN::get_onednn_strides(src);
...
static inline bool is_onednn_layout(const at::Tensor& tensor) {
  return !at::AtenTypeXPU::DPCPPTensorContext::is_plain(tensor);
}
...
static inline bool iteratable_has_onednn_layout(T container) {
  bool has_onednn_layout_tensor =
      std::any_of(container.begin(), container.end(), [](const Tensor& t) {
        return kernels::xpu::oneDNN::is_onednn_layout(t);
      });
...
static inline bool eltwise_forward_valid(const at::Tensor& tensor) {
  ...
  if (tensor.dim() > 6)
    return false;
  if (!at::AtenTypeXPU::DPCPPTensorContext::is_plain(tensor))
    return true;
...
static inline bool binary_valid(
  ...
  using namespace at::AtenTypeXPU;
...
static inline bool using_channels_last_for_onednn_op(const at::Tensor& input) {
  ...
  // format
  auto input_ctx =
      at::AtenTypeXPU::DPCPPTensorContext::get_tensor_ctx(input);
...
static inline Tensor contiguous_if_needed(
  ...
  auto ctx = at::AtenTypeXPU::DPCPPTensorContext::get_tensor_ctx(t);
...
static inline bool eltwise_forward_valid(
  ...
  if (!out.defined()) {
    ...
    if (!out.is_view() && out.is_contiguous() &&
      ...
      TORCH_CHECK(
          !kernels::xpu::oneDNN::is_onednn_layout(out),
...
static inline bool binary_forward_valid(
    ...
    const Tensor& other) {
  bool onednn_path_valid = true;
  if (!((kernels::xpu::oneDNN::is_onednn_layout(self) || kernels::xpu::oneDNN::is_onednn_layout(other)) &&
